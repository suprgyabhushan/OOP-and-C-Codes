...
char *errormsg[]={"Successful completion", 
		  ...
		  "Unknown unit",
		  "Bad argument",
		  "Weird function type (bug in program)",
		  "Function argument has wrong dimension",
		  "Argument of table outside domain",
		  "Function or table definition has unit error",
		  "No inverse defined",
		  "Parser memory overflow (recursive function definition?)",
		  "Argument wrong dimension or bad function definition"
                  };
...
readunits(char *userfile)
{
   ...
   free(line);
   if (!quiet)
     printf("%d units, %d prefixes, %d functions\n\n", unitcount, prefixcount,
	    funccount);
}
...
tryallunits(struct unittype *have)
{
  ...
  for(funcptr=firstfunc;funcptr;funcptr=funcptr->next){
    if (funcptr->table) 
      addtolist(have, funcptr->name, funcptr->tableunit, "<lookup table>", 
		&list, &listsize, &maxnamelen, &count);
    else
      addtolist(have, funcptr->name, funcptr->inverse.dimen, "<function>", 
		&list, &listsize, &maxnamelen, &count);
  }
